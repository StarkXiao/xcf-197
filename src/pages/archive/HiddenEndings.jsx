    ...
    return (
      <div
        ...
        className={`relative rounded-2xl p-4 border-2 transition-all duration-300 cursor-pointer
          ${isUnlocked
            ? 'bg-gradient-to-br from-star-purple/30 to-star-blue/30 hover:scale-105 hover:shadow-lg'
            : 'bg-gray-800/30 border-gray-700/30 opacity-50 cursor-not-allowed'
          }
        `}
        style={{
          borderColor: isUnlocked ? `${rarityColor}50` : undefined,
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className={`w-16 h-16 rounded-xl flex items-center justify-center text-4xl ${isUnlocked ? 'animate-float' : 'grayscale'}`}
            style={{
          ...
          {isUnlocked && (
            <div className="text-star-gold text-xl">✓</div>
          )}
        </div>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        ...
        <div className="flex justify-center gap-2 mb-6">
          ...
          >
            ...
            `}>
              {unlockedRewardsCount}/{rewards.length}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="mt-8 p-4 bg-star-purple/20 rounded-xl border border-star-gold/20">
        ...
        <ul className="text-sm text-white/60 space-y-1">
          ...
          <li>• 全章节3星通关可解锁「情书收藏家」结局</li>
          <li>• 达成各种成就可获得不同稀有度的奖励</li>
          <li>• 传说级奖励非常稀有，需要完成终极挑战</li>
        </ul>
      </div>

      {claimedNotification && (
        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 animate-bounce">
          <div className="bg-gradient-to-r from-star-gold to-amber-500 text-star-dark px-6 py-3 rounded-full shadow-lg shadow-star-gold/50 flex items-center gap-3">
            <span className="text-2xl">{claimedNotification.icon}</span>
            <span className="font-bold">获得奖励：{claimedNotification.name}！</span>
          </div>
        </div>
      )}
        ...
        {selectedEnding && (
          <div className="max-w-md mx-auto text-center">
            ...
            <div className="bg-gradient-to-r from-star-gold/20 to-amber-500/20 rounded-xl p-4 border border-star-gold/30">
              <div className="text-star-gold text-sm font-medium mb-2">🎁 已获得奖励</div>
              <p className="text-white font-bold">{selectedEnding.reward}</p>
            </div>

            <button
              onClick={() => {
                showRewardNotification({ icon: selectedEnding.icon, name: selectedEnding.reward });
      ...
      >
        {selectedReward && (
          <div className="max-w-md mx-auto text-center">
            ...
            <button
              onClick={() => {
                showRewardNotification(selectedReward);
